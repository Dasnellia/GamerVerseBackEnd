...
const EstadisticasController = () => {
    ...
    router.get("/ventas-por-mes", async (req: Request, resp: Response) => {
        try {
            const isAdmin = req.headers["admin"] === "true"
            if (!isAdmin) {
                resp.status(403).json({ msg: "Solo administradores pueden acceder a estos datos" })
                return
            }
            ...
            for (let mes = 0; mes < 12; mes++) {
                ...
                const total = ventas.reduce((sum: number, venta: Venta) => sum + parseFloat(venta.MontoPagado), 0)
    ...
    router.get("/ventas-hoy", async (req: Request, resp: Response) => {
        try {
            const isAdmin = req.headers["admin"] === "true"
            if (!isAdmin) {
                resp.status(403).json({ msg: "Solo administradores pueden ver estadísticas" })
                return
            }
            ...
            const totalHoy = ventasHoy.reduce((s : number, v: Venta) => s + parseFloat(v.MontoPagado), 0)
            const totalAyer = ventasAyer.reduce((s:number, v:Venta) => s + parseFloat(v.MontoPagado), 0)
